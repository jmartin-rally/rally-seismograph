...
module Rally
  module AppSdk
    ...
    class AppTemplateBuilder
      ...
      def replace_placeholder_variables(str, opts = {})
        ...
        str.gsub("APP_READABLE_NAME", @config.name).
            ...
            gsub("APP_SDK_PATH", debug ? @config.debug_sdk_path : @config.sdk_path).
            gsub("DEBUG_SDK_URL", @config.debug_sdk_url ).
            gsub("DEBUG_PROJECT_OID", @config.debug_project_oid).
            gsub("DEBUG_WORKSPACE_OID", @config.debug_workspace_oid).
    ...
    class AppConfig
      ...
      attr_accessor :debug_sdk_url, :debug_project_oid, :debug_workspace_oid
      ...
      def self.from_config_file(config_file)
        ...
        # John added for external running
        debug_sdk_url = Rally::RallyJson.get(config_file,"debugSDKAbsoluteURL") || "https://rally1.rallydev.com/apps"
        debug_project_oid = Rally::RallyJson.get(config_file,"debugProjectOID") || ""
        debug_workspace_oid = Rally::RallyJson.get(config_file,"debugWorkspaceOID") || ""
        
        config = Rally::AppSdk::AppConfig.new(name, sdk_version)
        config.javascript = javascript
        config.css = css
        config.class_name = class_name
        config.debug_sdk_url = debug_sdk_url
        config.debug_project_oid = debug_project_oid
        config.debug_workspace_oid = debug_workspace_oid
      ...
      def initialize(name, sdk_version)
        ...
        @debug_sdk_url = "https://rally1.rallydev.com/apps"
        @debug_project_oid = ""
        @debug_workspace_oid = ""
        @javascript = []
        @css = []
      ...
      def debug_sdk_path
        "#{@debug_sdk_url}/#{@sdk_version}/#{SDK_DEBUG_FILE}"
      end
    ...
    CONFIG_TPL = <<-END
{
    ...
    "className": "CustomApp",
    "sdk": "APP_SDK_VERSION",
    "debugSDKAbsoluteURL": "DEBUG_SDK_URL",
    "debugWorkspaceOID": "DEBUG_WORKSPACE_OID",
    "debugProjectOID": "DEBUG_PROJECT_OID",
    "javascript": [
        DEFAULT_APP_JS_FILE
    ],
    "css": [
